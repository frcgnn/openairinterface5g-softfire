...
move_steadystaterwp_node (pair_struct * pair, double cur_time)
{
  ...
  printf ("steadystaterwp node positions (X = %.2f Y = %.2f event_num =%d)", node->mob->x_from, node->mob->y_from, node->event_num);

  //initial move
  if (node->event_num == 0) {

     printf ("event_num =%d", node->event_num);
     ...
     */
      ...
      printf ("GRID event_num =%d", node->event_num);
      while (true) {
        pr = randomgen (0, 1);
        ...
        */
        ...
        node->mob->y_to =  (double) ((int) (node->mob->y_from));
      ...
      */
        ...
        temp_y = (double) ((int) (node->mob->y_from));  
        ...
        if (u1 <
          ...
          node->mob->y_from =  temp_y; 
          // + u2 * distance * sin (node->mob->azimuth);  
          //<Add:Giannone>
          break;
        }
      ...
      printf ("Not GRID event_num =%d", node->event_num);
       ...
       */ 
         ...
         printf ("While Not GRID event_num =%d", node->event_num); 
         node->mob->x_to =  (double) ((int) (1 + (node->mob->x_from*100)))/100;              
         node->mob->y_to =  (double) ((int) (node->mob->y_from));
               
         printf ("Node positions (X = %.2f Y = %.2f)", node->mob->x_to, node->mob->y_to);
        ...
        */
        ...
        if (u1 <
          ...
          u2 = randomgen (0, 1);
          node->mob->x_from = node->mob->x_to;
          node->mob->y_from = temp_y; //u2 * temp_y + (1 - u2) * node->mob->y_to;
    ...
    //<Add:Giannone>
    distance =  omg_param_list[node->type].myDistance ;
     ...
     /*(double) ((int)
                (sqrtf (pow (node->mob->x_from - node->mob->x_to, 2) +
                        pow (node->mob->y_from - node->mob->y_to,
                             2)) * 100)) / 100;
      */ 
    printf ("event_num =%d distance =%.2f speed =%.2f", node->event_num, distance, node->mob->speed); 
  ...
  //for the next move
     ...
     //printf ("Next move event_num =%d", node->event_num); 
    ...
    if (grid == CONNECTED_DOMAIN) {
      ...
      // printf ("Next move Not GRID event_num =%d", node->event_num); 
  ...
  //<Add:Giannone>
  journeytime_next = omg_param_list[node->type].myJourneytime_next;
  
  //(double) ((int) (distance / node->mob->speed * 100)) / 100;  //duration to get to dest
    
  //LOG_D (OMG, "[STEADY_RWP] mob->journey_time_next %.2f\n", journeyTime_next);
  printf ("[STEADY_RWP] mob->journey_time_next %.2f\n", journeytime_next);
  
  //</Add:Giannone>

  node->mob->start_journey = cur_time;
  //LOG_D (OMG, "[STEADY_RWP] start_journey %.2f\n", node->mob->start_journey);
  printf ("[STEADY_RWP] start_journey %.2f\n", node->mob->start_journey);
  ...
  printf ("[STEADY_RWP] reaching the destination at time : start journey + journey_time next =%.2f\n", pair->next_event_t);
