...
start_rwalk_generator (omg_global_param omg_param_list)
{
  ...
  for (id = n_id; id < (omg_param_list.nodes + n_id); id++) {
    ...
    printf ("Initial node positions (X = %.2f Y = %.2f)", node->mob->x_from, node->mob->y_from);
 ...
 */
  ...
  //<Add:Giannone> 
  node->x_pos = (double) omg_param_list[node->type].myUE_start_x;
  node->y_pos=  (double) omg_param_list[node->type].myUE_start_y;
  //</Add:Giannone>  
